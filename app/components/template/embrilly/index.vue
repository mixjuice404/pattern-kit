<template>
<div class="base-container embr-container prose" :style="{ padding: `${paddingY}px ${paddingX}px` }">
    <div class="mb-4">
        <div class="mb-8">
            <div class="title" style="margin-bottom: 22px;">{{ patternData.title ? patternData.title : 'Default Title (Required)' }}</div>
        ...
        <div class="mb-10" style="font-size: 12px; opacity: 0.8;">
            ...
            <div style="display: flex; align-items: center; gap:5px">
                <div>{{ L.websiteShop }}: </div>
                <img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Etsy_logo.svg" style="width: 30px; height: 14px; margin: 0; margin-top: 2px"/>
                <a href="https://www.etsy.com/shop/Embrilly">https://www.etsy.com/shop/Embrilly</a>
            </div>
        </div>
    ...
    <div class="section-block">
        <div class="divider-title">
            {{ patternData.introduction.title || L.beforeStartTitle }}
        </div>
        <div style="font-size: 15px; display: flex; flex-direction: column; gap: 15px">
            <div style="font-weight: 600">{{ L.beforeStartLead }}</div>
            <div v-html="L.beforeStartTip1"></div>
            <div>{{ L.beforeStartTip2 }}</div>
            <div>{{ L.beforeStartTip3 }}</div>
            <div v-html="L.beforeStartTip4"></div>
        </div>
        <div v-if="patternData.introduction.text" class="rich-text" style="margin-top: 15px; font-size: 14px;" 
        v-html="patternData.introduction.text"></div>
    </div>
    ...
    <div class="section-block">
        ...
        <div style="font-size: 15px" v-if="patternData.materialsDesc">
    ...
    <div class="section-block">
        ...
        <div v-for="(instructionGroup, groupIndex) in patternData.instructions" :key="groupIndex" class="instruction-group" style="margin-bottom: 40px;">
            <div style="font-size: 26px; font-weight: 700;" v-if="instructionGroup.title" class="group-title">{{ instructionGroup.title.toUpperCase() }}</div>
            ...
            <div v-for="(item, index) in instructionGroup.steps" :key="index" class="instruction-item">
                <div style="margin-top: 15px; font-size: 18px; font-weight: 700;margin-bottom: 5px;" v-if="item.title">{{ item.title }}</div>
                <div v-if="item.text">{{ item.text }}</div>
                <div v-if="item.description" class="description" style="font-size: 15px" v-html="item.description"></div>
                <div class="content-row" style="padding-bottom: 0;">
    ...
    <div class="section-block" v-if="patternData.finishingTips.list.length > 0">
        ...
        <ul style="font-size: 15px">
            <li v-for="(item, index) in patternData.finishingTips.list" :key="index" style="margin-bottom: 5px;">
                <template v-if="typeof item === 'string' && item.includes(':')">
                    <strong>{{ item.split(':')[0] }}</strong><div>{{ item.split(':')[1] }}</div>
                </template>
                <template v-else>
                    {{ item }}
                </template> 
